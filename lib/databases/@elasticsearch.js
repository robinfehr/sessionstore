...
async function clearOldIndices(client, index){
  ...
  indices.forEach(existingIndex=>{
    let matches = existingIndex.match(/([0-9]{4})-([0-9]{2})-([0-9]{2})$/) || [];
    if(matches !== 4){
      return;
    }
    let year = Number(matches[1]);
    ...
    if((now.getTime() - thirtyDaysMillis) >  indexDate.getTime()){
      client.indices.delete({existingIndex}); 
    }
...
_.extend(ElasticSearchSessionStore.prototype, {
  ...
  connect: function (callback) {
    ...
    setInterval(clearOldIndices, (1000 * 60 * 60 * 24));
