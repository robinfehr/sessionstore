...
_.extend(MongoSessionStore.prototype, {
  ...
  connect: function (callback) {
    ...
    if (mongo.MongoClient.length === 2) {
      self.client = new mongo.MongoClient(connectionUrl, options.options);
      self.client.connect(function(err, cl) {
        if (err) {
      ...
      self.client = new mongo.MongoClient();
      self.client.connect(connectionUrl, options.options, function(err, db) {
        if (err) {
          if (callback) callback(err);
          return;
        }
    ...
    function initDb() {
      ...
      var finish = function (err) {
        ...
        self.sessions.createIndexes({ expires: 1 }, { expireAfterSeconds: 0 }, function() {});
        if (!err) {
